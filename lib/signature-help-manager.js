...
module.exports = class SignatureHelpManager {
  ...
  async showDataTip(provider, editor, position) {
    try {
     ...
     if ((!signatureHelp) || (signatureHelp.signatures.length == 0)) { this.unmountDataTip(); }
     else {
      ...
      const signatureHelpView = new SignatureHelpView({ snippet: snippetHtml, html: documentationHtml });
  ...
  mountSignatureHelp(editor, position, view) {
    ...
    const overlayMarker = editor.markBufferRange(new Range(position, position), {
      invalidate: 'never', // TODO shouldn't this be surround
    });
