...
module.exports = class SignatureHelpManager {
  ...
  initialize(renderer) {
    ...
    this.subscriptions.add(
      ...
      atom.config.observe(
        "atom-ide-signature-help.showSignatureHelpOnTyping",
        (toggleSwitch) => {
  ...
  updateCurrentEditor(editor) {
    ...
    this.editorSubscriptions.add(
      this.editor.getBuffer().onDidChangeText((evt) => {
        ...
        const provider = this.providerRegistry.getProviderForEditor(
          this.editor
        );
    ...
    try {
      ...
      if (!signatureHelp || signatureHelp.signatures.length == 0) {
        ...
        if (parameter) {
          ...
          doc = signature.documentation.kind
        ...
        this.signatureHelpDisposables = this.mountSignatureHelp(
          editor,
          position,
          signatureHelpView
        );
  ...
  mountSignatureHelp(editor, position, view) {
    ...
    setTimeout(() => {
      view.element.style.bottom =
        editor.getLineHeightInPixels() +
        view.element.getBoundingClientRect().height +
        "px";
