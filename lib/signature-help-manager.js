  ...
  showDataTip(provider, editor, position) {
    provider.getSignatureHelp(editor, position)
        .then((result) => {
          ...
          let labelString = signature.label;

          if ((result.activeParameter != undefined) && (result.activeParameter != null)) {
            const param = (signature.parameters ? signature.parameters[result.activeParameter] : '');
            labelString = labelString.replace(param.label, `<b>${param.label}</b>`);
          }
          markedString.push(`<pre><code class="${editor.getGrammar().name.toLowerCase()}">${labelString}</code></pre>`);
