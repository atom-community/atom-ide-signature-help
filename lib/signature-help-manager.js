...
module.exports = class SignatureHelpManager {
  ...
  async showDataTip(provider, editor, position) {
    try {
     ...
     if ((!signatureHelp) || (signatureHelp.signatures.length == 0)) {
      ...
      const htmlString = this.makeHtmlFromSignature(signature, editor.getGrammar().name.toLowerCase());
  ...
  makeHtmlFromSignature(signature, grammarName) {
    ...
    result.push(`<pre><code class="${grammarName}">${signature.label}</code></pre>`);
